...
yaml_file_path = os.path.join(settings.BASE_DIR, 'kisdev.yaml')
...
class APIResp:
    ...
    def printAll(self):
        ...
        for x in self.getBody()._fields:
            body_info += f'\t-{x}: {getattr(self.getBody(), x)}\n'
        print(body_info)
        return body_info
...
def do_sell(stock_code, order_qty, order_price, prd_code="01", order_type="00"):
    response = do_order(stock_code, order_qty, order_price, buy_flag=False, order_type=order_type)
    return response
...
def do_buy(stock_code, order_qty, order_price, prd_code="01", order_type="00"):
    response = do_order(stock_code, order_qty, order_price, buy_flag=True, order_type=order_type)
    return response
...
def get_my_complete(sdt, edt=None, prd_code='01', zipFlag=True):
    ...
    if t1.isOK():
        ...
        # tdf.drop('', inplace=True)
        if (zipFlag):
            return tdf[['ord_dt','prdt_name','ord_dvsn_name', 'sll_buy_dvsn_cd_name', 'ord_qty', 'ord_unpr', 'avg_prvs','ord_tmd','tot_ccld_amt']]
        else:
            return tdf
    else:
        t1.printError()
